...
```python
#No, it is not possible to calculate the average of the column continent because it contains categorical data text values like "Europe", "Asia" not numerical data.
```
...
```python
#No, it is not meaningful to calculate the median of the continent column because it contains categorical data text values like "Europe", "Asia" not numerical data.
```
...
```python
std_speed = df_pokemon.groupby('Legendary')['Speed'].std()

mean_speed = df_pokemon.groupby('Legendary')['Speed'].mean()

cv_speed = std_speed / mean_speed
...
```python
legendary_skew = df_pokemon[df_pokemon['Legendary'] == True]['Speed'].skew()
non_legendary_skew = df_pokemon[df_pokemon['Legendary'] == False]['Speed'].skew()

print(f"Skewness of Speed for Legendary Pokémon: {legendary_skew:.4f}")
print(f"Skewness of Speed for Non-Legendary Pokémon: {non_legendary_skew:.4f}")

type1_skew = df_pokemon.groupby('Type 1')['Speed'].apply(skew)
print("\nSkewness of Speed by Type 1:")
print(type1_skew.sort_values(ascending=False))
```

    Skewness of Speed for Legendary Pokémon: 0.4424
    Skewness of Speed for Non-Legendary Pokémon: 0.3901
...
ıqr_kurtosis = (q3 - q1) / (2* (c90 - c10))

print("Regular Kurtosis:", ıqr_kurtosis)

```

    Regular Kurtosis: 0.24999999999999997
...
sns.scatterplot(data=df_pokemon, x='Attack', y='Speed', hue='Legendary', alpha=0.7)

plt.title('Speed vs Attack by Legendary Status')
plt.xlabel('Attack')
plt.ylabel('Speed')
...
sns.scatterplot(data=df_pokemon, x='Defense', y='Speed', hue='Generation', palette='Set2', alpha=0.7)

plt.title('Speed vs Defense by Generation')
plt.xlabel('Defense')
plt.ylabel('Speed')
...
![png](Exercise8_files/Exercise8_104_0.png)
    



    
![png](Exercise8_files/Exercise8_104_1.png)
